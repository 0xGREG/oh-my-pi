import { centerLine, fitLine, visibleWidth } from "../../utils";
...
function renderCompactSplash(width: number, height: number, phase: number, shine: ShineConfig): string[] {
	...
	for (let y = 0; y < height; y++) {
		...
		lines.push(fitLine(item !== undefined ? centerLine(item, width) : "", width));
	}
	if (height > 2) lines[height - 2] = fitLine(centerLine(theme.fg("dim", SKIP_HINT), width), width);
	return lines;
}
