...
- Fixed `/login` crashing source-link and dev installs with `NameTooLong reading "file:file:…"` once an extension loader had loaded. The legacy-pi specifier shim no longer re-resolves a canonical `@oh-my-pi/pi-*` subpath such as `@oh-my-pi/pi-ai/index.js` that maps to itself, so `require()` of those subpaths falls through to Bun's native resolution ([#12293](https://github.com/can1357/oh-my-pi/issues/12293)).
- Fixed `/login` crashing source-link and dev installs with `NameTooLong reading "file:file:…"` once an extension loader had loaded. The legacy-pi specifier shim no longer re-resolves a canonical `@oh-my-pi/pi-*` subpath such as `@oh-my-pi/pi-ai/index.js` that maps to itself, so `require()` of those subpaths falls through to Bun's native resolution ([#12293](https://github.com/can1357/oh-my-pi/issues/12293), [#13127](https://github.com/can1357/oh-my-pi/pull/13127) by [@alnaggar-dev](https://github.com/alnaggar-dev)).
