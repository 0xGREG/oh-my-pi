...
- `path`: one directory to search, or an `omp://` docs scope (`omp://` for all harness docs, `omp://<file>.md` for one); omit for the workspace root. Narrow it when you already know the subsystem — fewer files to rank means cheaper, sharper results.
