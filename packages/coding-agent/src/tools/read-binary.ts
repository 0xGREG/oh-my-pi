...
import { acquireIdaDatabase, cfgIdaAvailable, isExecutableFile, isIdaDatabasePath } from "../ida";
...
export async function resolveBinaryViewPath(session: ToolSession, readPath: string): Promise<BinaryViewTarget | null> {
	...
	for (let i = readPath.lastIndexOf(":"); i > 0; i = readPath.lastIndexOf(":", i - 1)) {
		...
		if (isIdaDatabasePath(absolutePath) || (await isExecutableFile(absolutePath))) {
			return { absolutePath, view: readPath.slice(i + 1) };
		}
	}
	return null;
}
...
export async function readBinary(
	...
	const db = await acquireIdaDatabase(session, target.absolutePath, signal);
	const { text } = await db.request<{ text: string }>("view", view, { signal, timeoutMs: VIEW_TIMEOUT_MS });
	let output = text;
	if (view.kind === "overview") {
		const p = formatPathRelativeToCwd(target.absolutePath, session.cwd);
		output = `[${db.id}; views: ${p}:<func|0xaddr> pseudocode, ${p}:<func>:asm, :imports, :exports, :strings, :xrefs:<func|0xaddr>]\n${text}`;
	}
