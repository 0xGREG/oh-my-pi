...
- **Provider adapters**
  - **Perplexity** — `packages/coding-agent/src/web/search/providers/perplexity.ts`
    ...
    - SDK hosts can provide `OAuthController.onBrowserSession` to capture the requested cookie privately; pi-ai validates it without importing browser automation. Hosts without that callback retain the email-code flow. RPC login does not launch a browser.
